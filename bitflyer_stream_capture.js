...
var startFileDate = moment().format("YYYYMMDDHHmm");
...
streaming.subscribeExecutionsFx(function (err,message) {
	...
	// 記録
	fs.appendFile('data/execution_FX_BTC_JPY.' + startFileDate + ".dat", data, (err) => {
		if(err){
...
streaming.subscribeTickerFx(function (err,message) {
	...
	// 記録
	fs.appendFile('data/ticker_FX_BTC_JPY.' + startFileDate + ".dat", data, (err) => {
		if(err){
			console.log("append data fail!",err);
		}
	});
...
streaming.subscribeBoardSnapshotFx(function (err,message) {
	...
	// 記録
	fs.appendFile('data/board_snapshot_FX_BTC_JPY.' + startFileDate + ".dat", data, (err) => {
		if(err){
			console.log("append data fail!",err);
		}
	});
...
streaming.subscribeBoardFx(function (err,message) {
	...
	// 記録
	fs.appendFile('data/board_FX_BTC_JPY.' + startFileDate + ".dat", data, (err) => {
		if(err){
			console.log("append data fail!",err);
		}
	});
